  ...
  const fetchSalesData = async () => {
    ...
    try {
      const response = await fetch('https://us-central1-myposdata.cloudfunctions.net/sales', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'getSalesDataByBillNumberAndDate', billNumber, userEmail })
      });
      const data = await response.json();
      if (data.error) throw new Error(data.error);
      ...
      <div id="salesPrintPreview">
        {salesData ? (
          <div>
            ...
            <p>Mobile Number: {salesData.salesData[0]?.Mobile_Number || ''}</p>
            <table className="sales-table">
              <thead>
                <tr>
                  <th>Item</th>
              ...
              <tbody>
                {salesData.salesData.map(row => (
                  <tr key={row.SaleData_ID || row.Item}>
                    <td>{row.Item || ''}</td>
            ...
            {salesData.paymentData && salesData.paymentData.length > 0 && (
              <div>
                <h3>Payment Data</h3>
                <table className="sales-table">
                  <thead>
                  ...
                  <tbody>
                    {salesData.paymentData.map(row => (
                      <tr key={row.PaymentData_ID || row.Paymode}>
                        <td>{row.Paymode || ''}</td>
